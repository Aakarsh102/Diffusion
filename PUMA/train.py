    ...
    for epoch in range(train_cfg.num_epochs):
        ...
        for itr in pbar:
            ...
            if global_step % train_cfg.eval_steps == 0:
                ...
                if is_main:
                    L = model_config.max_position
                    x_t = torch.full((4, L), mask_id, dtype=torch.long, device=device)

                    model_to_sample = model.module if isinstance(model, DDP) else model
                    samples = mdm_sampling(model_to_sample, x_t, mask_id, val_cfg.sampling, device=device)
                    ...
                    print(samples.detach().cpu())
                #x_t = torch.full((4, 512), mask_id).to(device)
                #x_t = mdm_sampling(model, x_t, mask_id)
                #print("generated samples:")
